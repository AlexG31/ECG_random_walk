...
class ECGfeatures(object):
    ...
    def __init__(
        ...
        if not isinstance(rawsig, list) and not isinstance(rawsig,
                array.array) and not isinstance(rawsig, np.ndarray):
        ...
        self.fixed_window_length = self.config['fs'] \
            * self.config['winlen_ratio_to_fs']
    ...
    def crop_data_for_swt(self, rawsig):
        ...
        N_data = len(rawsig)
        if len(rawsig) <= 1:
            raise Exception('len(rawsig)={}, not enough for swt!',
                            len(rawsig))
        crop_len = base2
        while base2 < N_data:
            if base2 * 2 >= N_data:
    ...
    def GetWindowedMatrix(self, position, cDlist = None, cAlist = None):
        ...
        for detail_coefficients in cDlist:
            windowed_matrix.append(self.getWindowedSignal(position,
                                   detail_coefficients,
                                   fixed_window_length))
    ...
    def getRandomPatterns(self):
        '''Return the list of random patterns in configuration.'''
