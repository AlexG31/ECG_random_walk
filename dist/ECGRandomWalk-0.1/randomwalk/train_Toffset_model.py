...
def TrainingOnChanggeng(walker, target_label, model_file_name, training_list):
    annots_folder = '/home/alex/LabGit/ECG_random_walk/tools/annotations/changgeng/Toffset/'
    import glob
    training_list = glob.glob(annots_folder + '*.json')
    ...
    for file_name in training_list:
        with open(file_name, 'r') as fin:
            ...
            cID = data['ID']
            record_name = cID
            annots = zip(data['poslist'], [data['type'], ] * len(data['poslist']))
        if len(annots) == 0:
            continue
            
        print 'Collecting features from changgeng record %s.' % record_name
        sig = cloader.loadID(cID)
        import scipy, scipy.signal
        # Resample to 250Hz
        fs = 500.0
        sig = scipy.signal.resample(sig, int(len(sig) / float(fs) * 250.0))
        for annot_index in xrange(0, len(annots)):
            annots[annot_index] = [annots[annot_index][0] * 250.0 / fs,
                    annots[annot_index][1]]
        
        walker.collect_training_data(sig,
                annots,
                )
...
def TrainingModels(target_label, model_file_name, training_list):
    ...
    # Training with changgeng data
    TrainingOnChanggeng(walker, target_label, model_file_name, training_list)
    
    for record_name in training_list:
        expert_marks = qt.getExpert(record_name)
        Tonset_marks = [x for x in expert_marks if x[1] == 'Toffset']
        if len(Tonset_marks) == 0:
            continue
            
        print 'Collecting features from record %s.' % record_name
        sig = qt.load(record_name)
        walker.collect_training_data(sig['sig'],
                Tonset_marks,
                )
    ...
    walker.training()
    print 'training used %.3f seconds' % (time.time() - start_time)
